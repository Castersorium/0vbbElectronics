        ...
        for ( size_t entryIndex = 0; entryIndex < n; ++entryIndex )
        {
            ...
            if ( timestamp < timestamp_ini || timestamp >= timestamp_ini + timeWindow )
            {
                ...
                if ( hist )
                {
                    ...
                    // 计算标准差
                    double sum_deviation_X = std::accumulate( timestamps_vec.begin(), timestamps_vec.end(), 0.0, [average_X]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - average_X ) * ( val - average_X );
                                                              } );
                    double sum_deviation_Y = std::accumulate( Amp_vec.begin(), Amp_vec.end(), 0.0, [maxX]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - maxX ) * ( val - maxX );
                                                              } );
        ...
        for ( size_t entryIndex = 0; entryIndex < n; ++entryIndex )
        {
            ...
            if ( timestamp < timestamp_ini || timestamp >= timestamp_ini + timeWindow )
            {
                if ( !timestamps_vec.empty() )
                {
                    ...
                    double sum_deviation_Y = std::accumulate( temperatures_vec.begin(), temperatures_vec.end(), 0.0, [average_Y]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - average_Y ) * ( val - average_Y );
                                                              } );
                    double stddev_X = std::sqrt( sum_deviation_X / timestamps_vec.size() );
...
// 从TTree创建TGraphErrors并保存到ROOT文件的方法
void TTreePlotter::createBlueforsPressureGraphFromTree( const std::string & rootFilePath, const std::string & outputFilePath ) const
{
    // 打开ROOT文件
    TFile * file = TFile::Open( rootFilePath.c_str(), "READ" );
    if ( !file || file->IsZombie() )
    ...
    // 从文件中获取TTree
    TTree * tree = dynamic_cast<TTree *>( file->Get( "PressureReadings" ) );
    if ( !tree )
    {
        std::cerr << "Error: Cannot find TTree in ROOT file: " << rootFilePath << std::endl;
        return;
    ...
    for ( size_t i = 0; i < nBranches; ++i )
    {
        ...
        tree->SetBranchAddress( "timestamp", &timestamp );
        tree->SetBranchAddress( branchName.c_str(), &pressure );

        // 创建一个向量来存储每个时间窗口内的所有时间戳
        std::vector<double> timestamps_vec;
        std::vector<double> pressures_vec;
        double timestamp_ini = -1.0; // 初始的时间戳
        ...
        for ( size_t entryIndex = 0; entryIndex < n; ++entryIndex )
        {
            ...
            if ( timestamp < timestamp_ini || timestamp >= timestamp_ini + timeWindow )
            {
                if ( !timestamps_vec.empty() )
                {
                    // 计算时间戳的平均值并打印出来
                    double average_X = std::accumulate( timestamps_vec.begin(), timestamps_vec.end(), 0.0 ) / timestamps_vec.size();
                    double average_Y = std::accumulate( pressures_vec.begin(), pressures_vec.end(), 0.0 ) / pressures_vec.size();
                    ...
                    double sum_deviation_Y = std::accumulate( pressures_vec.begin(), pressures_vec.end(), 0.0, [average_Y]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - average_Y ) * ( val - average_Y );
                                                              } );
                    double stddev_X = std::sqrt( sum_deviation_X / timestamps_vec.size() );
                    double stddev_Y = std::sqrt( sum_deviation_Y / pressures_vec.size() );
                    ...
                    if ( isDebugModeActive )
                    {
                        ...
                        TDatime date( static_cast<UInt_t>( average_X ) );
                        std::cout << "Average date and time for " << static_cast<int>( timestamp_ini ) << ": " << date.AsString() << std::endl;
                        std::cout << "Average pressure for " << static_cast<int>( timestamp_ini ) << ": " << average_Y << std::endl;
                        std::cout << "Standard deviation of pressure for " << static_cast<int>( timestamp_ini ) << ": " << stddev_Y << std::endl;
                    }
            ...
            if ( pressure != 0.0 )
            {
                // 将当前的时间戳添加到向量中
                timestamps_vec.emplace_back( timestamp );
                pressures_vec.emplace_back( pressure );
            }
        }
    ...
    // 创建一个TCanvas
    TCanvas * canvas = new TCanvas( "canvas_Pressure", "canvas_Pressure", 1600, 900 );
    ...
    // 创建一个MultiGraph
    TMultiGraph * multiGraph = new TMultiGraph( "mg_Pressure", "Pressure of P1_CHAMBER, P2_STILL, P3_CONDENSING, P4_TRAP, P5_TANK and P6_SERVICE" );
    ...
    multiGraph->GetXaxis()->SetTitle( "Time" );
    multiGraph->GetYaxis()->SetTitle( "Pressure [mbar]" );
    multiGraph->GetXaxis()->SetRangeUser( xTimeStampMin - 60, xTimeStampMax + 60 );
    multiGraph->GetXaxis()->SetTimeDisplay( 1 );
    // 设置X轴上的时间格式
    // multiGraph->GetXaxis()->SetTimeFormat( "%a %d/%m/%y %H:%M:%S" );
...
// 从TTree创建TGraphErrors并保存到ROOT文件的方法
void TTreePlotter::createBlueforsFlowGraphFromTree( const std::string & rootFilePath, const std::string & outputFilePath ) const
{
    // 打开ROOT文件
    TFile * file = TFile::Open( rootFilePath.c_str(), "READ" );
    if ( !file || file->IsZombie() )
    ...
    // 从文件中获取TTree
    TTree * tree = dynamic_cast<TTree *>( file->Get( "FlowReadings" ) );
    if ( !tree )
    {
        std::cerr << "Error: Cannot find TTree in ROOT file: " << rootFilePath << std::endl;
        return;
    ...
    for ( size_t i = 0; i < nBranches; ++i )
    {
        ...
        tree->SetBranchAddress( "timestamp", &timestamp );
        tree->SetBranchAddress( branchName.c_str(), &flow );

        // 创建一个向量来存储每个时间窗口内的所有时间戳
        std::vector<double> timestamps_vec;
        std::vector<double> flows_vec;
        double timestamp_ini = -1.0; // 初始的时间戳
        ...
        for ( size_t entryIndex = 0; entryIndex < n; ++entryIndex )
        {
            ...
            if ( timestamp < timestamp_ini || timestamp >= timestamp_ini + timeWindow )
            {
                if ( !timestamps_vec.empty() )
                {
                    // 计算时间戳的平均值并打印出来
                    double average_X = std::accumulate( timestamps_vec.begin(), timestamps_vec.end(), 0.0 ) / timestamps_vec.size();
                    double average_Y = std::accumulate( flows_vec.begin(), flows_vec.end(), 0.0 ) / flows_vec.size();
                    ...
                    double sum_deviation_Y = std::accumulate( flows_vec.begin(), flows_vec.end(), 0.0, [average_Y]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - average_Y ) * ( val - average_Y );
                                                              } );
                    double stddev_X = std::sqrt( sum_deviation_X / timestamps_vec.size() );
                    double stddev_Y = std::sqrt( sum_deviation_Y / flows_vec.size() );
                    ...
                    if ( isDebugModeActive )
                    {
                        ...
                        TDatime date( static_cast<UInt_t>( average_X ) );
                        std::cout << "Average date and time for " << static_cast<int>( timestamp_ini ) << ": " << date.AsString() << std::endl;
                        std::cout << "Average flow for " << static_cast<int>( timestamp_ini ) << ": " << average_Y << std::endl;
                        std::cout << "Standard deviation of flow for " << static_cast<int>( timestamp_ini ) << ": " << stddev_Y << std::endl;
                    }
    ...
    // 创建一个TCanvas
    TCanvas * canvas = new TCanvas( "canvas_Flow", "canvas_Flow", 1600, 900 );
    ...
    // 创建一个MultiGraph
    TMultiGraph * multiGraph = new TMultiGraph( "mg_Flow", "Flow of MIXTURE" );
    ...
    multiGraph->GetXaxis()->SetTitle( "Time" );
    multiGraph->GetYaxis()->SetTitle( "Flow [mmol/s]" );
    multiGraph->GetXaxis()->SetRangeUser( xTimeStampMin - 60, xTimeStampMax + 60 );
    multiGraph->GetXaxis()->SetTimeDisplay( 1 );
    // 设置X轴上的时间格式
    // multiGraph->GetXaxis()->SetTimeFormat( "%a %d/%m/%y %H:%M:%S" );
        ...
        for ( size_t entryIndex = 0; entryIndex < n; ++entryIndex )
        {
            ...
            if ( timestamp < timestamp_ini || timestamp >= timestamp_ini + timeWindow )
            {
                if ( !timestamps_vec.empty() )
                {
                    ...
                    double sum_deviation_Y = std::accumulate( readings_vec.begin(), readings_vec.end(), 0.0, [average_Y]( auto sum, auto val )
                                                              {
                                                                  return sum + ( val - average_Y ) * ( val - average_Y );
                                                              } );
                    double stddev_X = std::sqrt( sum_deviation_X / timestamps_vec.size() );
...
// 从TTree创建TGraphErrors并保存到ROOT文件的方法
void TTreePlotter::createR_BaseGraphFromTree( const std::string & rootFilePath, const std::string & outputFilePath ) const
{
    // 打开ROOT文件
    TFile * file = TFile::Open( rootFilePath.c_str(), "READ" );
    if ( !file || file->IsZombie() )
    ...
    // 从文件中获取TTree
    TTree * tree = dynamic_cast<TTree *>( file->Get( "R_BaseReadings" ) );
    if ( !tree )
    {
        std::cerr << "Error: Cannot find TTree in ROOT file: " << rootFilePath << std::endl;
        return;
    ...
    // 创建一个TCanvas
    TCanvas * canvas = new TCanvas( "canvas_R_BaseReading", "canvas_R_BaseReading", 1600, 900 );
    ...
    // 将TCanvas写入文件
    canvas->Write( "canvas_R_BaseReading" );
