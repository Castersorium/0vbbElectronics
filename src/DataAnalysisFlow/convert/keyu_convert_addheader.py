...
def process_tdms_files(input_path, output_path):
    ...
    files = [f for f in os.listdir(input_path) if f.endswith(".tdms") and "05-11" in f ]
    ...
    for file in files:
        ...
        #if os.path.getsize(fname) > 80 * 1024 * 1024 or os.path.getsize(fname) < 70 * 1024 * 1024:  # 1GB = 1 * 1024^3 bytes
        if os.path.getsize(fname) < 1* 1024 * 1024 * 1024 :  # 1GB = 1 * 1024^3 bytes
            continue 
...
input_path = "/mnt/d/RUNs/RUN2504/USTC/USTC-LD/NI Project Data"
...
output_path = "/mnt/d/RUNs_data_analysis/USTC-LD"
