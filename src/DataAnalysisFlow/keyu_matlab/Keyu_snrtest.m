...
valid_indices = (time >= 9100 & time <= 9400); % 替换为所需时间范围
...
for i = 2:length(group_start)
    ...
    if time_diff > segment_length
        ...
        group_end_final = [group_end_final; new_group_end - stable_time2*freq];
    end
...
for i = 1:num_segments
    ...
    mean_peaks(i) = mean(pks_without_max);
        ...
        pks3(i) = mean(pks(pks < (mean(pks) + 2 * std(pks))));
...
for i = 1:length(group_start_final)
    ...
    yline(th{i}, '--', sprintf('Threshold %d', i), 'Color', 'c', 'LineWidth', 1.5);
    yline(mean_peaks(i), '--', sprintf('峰值1 %d', i), 'Color', 'b', 'LineWidth', 1.5);
    yline(p_const(i), '--', sprintf('峰值2 %d', i), 'Color', 'y', 'LineWidth', 1.5);
    yline(pks3(i), '--', sprintf('峰值3 %d', i), 'Color', 'k', 'LineWidth', 1.5);
...
plot(voltage_odd, mean_baseline(1:2:end), '-o', 'LineWidth', 2, 'DisplayName', 'Odd Indices');
